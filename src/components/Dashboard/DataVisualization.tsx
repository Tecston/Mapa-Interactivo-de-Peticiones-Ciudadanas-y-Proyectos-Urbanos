...
const DataVisualization = () => {
  ...
  const categoryData = requests.reduce((acc: {
    name: string;
    value: number;
  }[], request) => {
    const existingCategory = acc.find(item => item.name === request.category);
    if (existingCategory) {
  ...
  const supportData = requests.map(request => ({
    name: request.title.length > 20 ? request.title.substring(0, 20) + '...' : request.title,
  ...
  return <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-gray-50 p-4 rounded-lg md:col-span-2">
          <h3 className="text-lg font-semibold mb-4">Apoyos por Petición</h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={supportData} margin={{
              top: 5,
              right: 30,
              left: 20,
              bottom: 5
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="apoyos" fill="#8884d8" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
          ...
          <p className="text-3xl font-bold">
            {requests.reduce((acc, req) => acc + req.supports, 0) + projects.reduce((acc, proj) => acc + proj.likes + proj.dislikes, 0)}
          </p>
        </div>
      </div>
    </div>;
